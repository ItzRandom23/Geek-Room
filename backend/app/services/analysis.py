...
def build_recommendations(events: list[Event], correlations: list[dict], transcript: str) -> list[dict]:
    ...
    if any(event.confidence < 0.5 for event in events):
        recommendations.append({"type": "review", "severity": "low", "title": "Manual review recommended", "explanation": "At least one reportable vocal-state event has low confidence.", "recommendation": "Use the original-language transcript and audio player to verify the event before acting.", "supporting_data": {"low_confidence_events": sum(event.confidence < 0.5 for event in events)}})
