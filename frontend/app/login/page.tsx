    ...
    <main className="mx-auto grid min-h-[calc(100vh-128px)] max-w-7xl items-center gap-12 px-6 py-12 lg:grid-cols-[1fr_440px]">
      ...
      <section className="telemetry-shell w-full p-6 sm:p-8">
        ...
        <form onSubmit={submit} className="mt-6 space-y-4">
          ...
          <label className="block text-xs text-slate-400"><span className="flex items-center gap-2"><LockKeyhole size={13} />Password</span><input required minLength={8} type="password" autoComplete={register ? "new-password" : "current-password"} placeholder="8+ characters" value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} className="field mt-2" /></label>
          <Button className="w-full" disabled={busy}>{busy ? (register ? "Creating..." : "Signing in...") : (register ? "Create team account" : "Sign in")}</Button>
        </form>
        <button type="button" onClick={() => { setRegister(!register); setError(""); }} className="mt-5 w-full text-sm font-medium text-cyan transition hover:text-white">
